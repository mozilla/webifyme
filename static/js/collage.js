...
things.Obj = function(pixelWidth, pixelHeight) {
  ...
  that.setSize = function(pWidth, pHeight) {
    that.pixelWidth = pWidth;
    that.pixelHeight = pHeight;
...
things.SnapshotChecker = function() {
  ...
  function doPoll() {
    $.ajax({'method':'get','url':pollUrl,'success':onSuccess,'error':onError,'dataType':'json'});
  }
  
  function setSnapshot(url) {
    enabled = true;
  ...
  function init() {
    if(things.snapshotUrl && things.snapshotUrl != '') {
      setSnapshot(things.snapshotUrl);
      return;
    }
